...
# Declared components for 42-Command-Center (Next.js 16 project)
DECLARED_COMPONENTS = {
    "src/app":              ("src/app/",              ["ts","tsx"],   True),   # App Router — main entry
    "src/components":       ("src/components/",       ["ts","tsx"],   False),  # React components
    "src/hooks":            ("src/hooks/",            ["ts"],         False),
    "src/lib":              ("src/lib/",              ["ts"],         True),   # core libs (auth, db, factions, game logic)
    "mini-services":        ("mini-services/",        ["ts"],         True),   # socket.io game engine on :3003
    "prisma":               ("prisma/",               [],             False),  # DB schema
    "scripts":              ("scripts/",              ["ts","js"],    False),
    "docs":                 ("docs/",                 [],             False),
    "public":               ("public/",               [],             False),
}
...
def main() -> int:
    ...
    lines.append("|---|---|---|")
    bible_critical = {
        "Game engine tick loop": ("tick", "mini-services"),
        "Faction system (FANG/HAMMER/RESOLUTE)": ("faction", "src/lib"),
        "Auth (Telegram + wallet + dev-passgate)": ("auth", "src/lib"),
        "Socket.io real-time transport": ("socket", "mini-services"),
        "MapLibre 3D globe": ("map", "src/components"),
        "Prisma persistence": ("prisma", "prisma"),
        "ARIA AI co-pilot": ("aria", "src/app/api"),
    }
