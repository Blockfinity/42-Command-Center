...
// ?title=…&visibility=unlisted — and forwards to the backend server-side,
// so the response can hand back ready-to-use www URLs.
export async function POST(request: NextRequest) {
  const contentType = request.headers.get("content-type") ?? "";
  const rawBody = await request.text();
  const query = request.nextUrl.searchParams;

  let payload: { title: string; content: string; content_type: string; visibility: string };
